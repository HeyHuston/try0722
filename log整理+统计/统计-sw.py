...
document=Document('sw2.docx')  
for no in range(0,len(document.paragraphs)):
    text=document.paragraphs[no].text    
    pc='';jn='';jnz='' 
    flag0='检定：'in text;flag00='成长检定：'in text;flag000='[惩罚骰:'in text
    if (flag0==True)and(flag00==False):    
        #i=text.find('：')
        m=text.find('的')
        n=text.find('检定：')
        p=text.find('/')
        ...
        dcg='大成功' in text
        dsb='大失败' in text
        pc=text[:m];jn=text[m+1:n];jnz=text[p+1:q]
        #pc=gaiming(pc)
        flag=' 'in pc
        if (flag==True):
            continue
        else:          
            if any(pc == pcs for pcs in pclist):
                over=''
            else:
                pclist.append(pc)
                print (pc)
                pcdic[pc]=DFlist[num]
                DFlist[num].loc['惩罚骰']=['/',0,0,0,0,0]
                num=num+1
            DF=pcdic[pc]
            if any(jn == jns for jns in DF.index):
                ...
                if (dsb==True):
                    DF.at[jn,'大失败次数']=DF.at[jn,'大失败次数']+1
                DF.at[jn,'成功率']=DF.at[jn,'成功次数']/DF.at[jn,'尝试次数']
    elif (flag000==True)and(flag00==False):  
        i=text.find('掷骰')
        pc=text[:i]
        if any(pc == pcs for pcs in pclist):
            over=''
            ...
            num=num+1
        DF=pcdic[pc]
        DF.at['惩罚骰','尝试次数']=DF.at['惩罚骰','尝试次数']+1
            
            

pcdic['雅克'].to_excel("sw.xlsx",sheet_name='1')       
for pc in pclist:
    DF=pcdic[pc]
    #add=str(pc)+"表.xlsx"
    #DF.to_excel("表.xlsx",sheet_name=pc)
    book = load_workbook('sw.xlsx')
    writer = pd.ExcelWriter("sw.xlsx",engine='openpyxl')
    writer.book = book
